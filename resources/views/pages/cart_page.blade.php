...
<style>
.main-cart-item { display: flex; align-items: center; justify-content: space-between; padding: 20px 0; border-bottom: 1px solid rgba(0,0,0,0.05); gap: 15px; }
.main-cart-item:last-child { border-bottom: none; }
.main-cart-img { width: 90px; height: 90px; object-fit: cover; border-radius: 18px; }
.page-quantity-box { display: flex; align-items: center; background: #fff0f6; border-radius: 12px; padding: 4px; }
.page-quantity-box button { border: none; background: transparent; width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; color: #ff2d7a; font-weight: bold; font-size: 16px; border-radius: 8px; transition: 0.2s; cursor: pointer; }
.page-quantity-box button:hover { background: rgba(255,45,122,0.1); }
.page-quantity-box span { width: 35px; text-align: center; font-weight: 600; color: #111; }
.checkout-btn:hover { background: #e02066 !important; transform: translateY(-2px); box-shadow: 0 12px 28px rgba(255,45,122,0.35) !important; }
@media(max-width: 767.98px) {
    .main-cart-item { flex-direction: column; align-items: flex-start; }
    .main-cart-right { width: 100%; display: flex; justify-content: space-between; align-items: center; margin-top: 10px; }
}
</style>

<script>
document.addEventListener("DOMContentLoaded", function() { renderPageCart(); window.addEventListener('cartUpdated', function() { renderPageCart(); }); });
...
function renderPageCart() {
    ...
    if(localCart.length === 0) {
        ...
        if(paymentBtn) { paymentBtn.style.pointerEvents = "none"; paymentBtn.style.opacity = "0.5"; }
        return;
    }
    if(paymentBtn) { paymentBtn.style.pointerEvents = "auto"; paymentBtn.style.opacity = "1"; }
        ...
        targetContainer.innerHTML += `
            <div class="main-cart-item">
                <div class="d-flex align-items-center gap-3">
                    <img src="${item.image}" class="main-cart-img" alt="food">
                    <div>
                ...
                <div class="main-cart-right d-flex align-items-center gap-4">
                    ...
                    <button onclick="deletePageItem(${index})" class="border-0 bg-transparent text-danger fw-bold" style="font-size:22px; line-height:1; cursor:pointer;">×</button>
                </div>
            </div>
        `;
    });
...
function modifyPageQty(index, offset) {
    ...
    if(targetQty <= 0) { localCart.splice(index, 1); } else { localCart[index].quantity = targetQty; }
