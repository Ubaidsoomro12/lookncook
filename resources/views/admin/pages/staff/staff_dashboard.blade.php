    ...
    <div class="card border-0 shadow-sm mb-4" style="background: linear-gradient(135deg, #1e1e2d, #2d2d3f);">
        <div class="card-body d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div class="d-flex align-items-center gap-3">
                <div style="width:44px;height:44px;border-radius:12px;background:rgba(255,45,122,0.15);display:flex;align-items:center;justify-content:center;">
                    <i class="fa-regular fa-clock" style="color:#ff2d7a;font-size:18px;"></i>
                </div>
                <div>
                    <div style="color:#9ca3af;font-size:11px;text-transform:uppercase;letter-spacing:0.5px;font-weight:600;">Current Time (PKT)</div>
                    <div id="liveClock" style="color:#fff;font-size:22px;font-weight:700;font-family:monospace;">--:--:-- --</div>
                </div>
            </div>
            <div class="text-end">
                <div style="color:#9ca3af;font-size:11px;text-transform:uppercase;letter-spacing:0.5px;font-weight:600;">Today</div>
...
<div class="row g-4">
    ...
    <div class="col-md-4">
        <div class="card border-0 shadow-sm h-100">
            <div class="card-body">
                <h6 class="card-title text-muted"><i class="fa-regular fa-user-clock me-2"></i> Attendance Status</h6>
                @if($isReopened)
                ...
                <p class="text-muted small mb-2">
                    @if(!$isReopened)
                        @if($clockIn)
                            <i class="fa-regular fa-right-to-bracket text-success me-1"></i>
                            In: <strong>{{ \Carbon\Carbon::parse($clockIn)->format('h:i:s A') }}</strong>
                        @else
                            Not clocked in yet.
                        @endif
                        @if($clockOut)
                            <br><i class="fa-regular fa-right-from-bracket text-danger me-1"></i>
    ...
    <div class="col-md-4">
        @if($showClockOutButton)
            {{-- LIVE WORKING TIMER --}}
            <div class="card border-0 shadow-sm h-100" style="background: linear-gradient(135deg, #22c55e, #16a34a); color:#fff;">
                <div class="card-body text-center d-flex flex-column justify-content-center">
                    ...
                    <h1 class="fw-bold mb-2" id="workingTimer" style="font-family:monospace;font-size:2.5rem;text-shadow:0 2px 4px rgba(0,0,0,0.2);">00:00:00</h1>
                    <div class="text-white-50 small">
                        Started at <strong style="color:#fff;">{{ \Carbon\Carbon::parse($clockIn)->format('h:i:s A') }}</strong>
                    </div>
                    <div class="mt-2">
        ...
        @elseif($showCompleted)
            ...
            @endphp
            <div class="card border-0 shadow-sm h-100" style="background: linear-gradient(135deg, #3b82f6, #2563eb); color:#fff;">
                <div class="card-body text-center d-flex flex-column justify-content-center">
                    <h6 class="text-white-50 mb-2" style="font-size:12px;text-transform:uppercase;letter-spacing:1px;">
                        <i class="fa-solid fa-circle-check me-1"></i> Total Worked
                    </h6>
                    <h1 class="fw-bold mb-2" style="font-family:monospace;font-size:2.5rem;text-shadow:0 2px 4px rgba(0,0,0,0.2);">
                        {{ sprintf('%02d:%02d:%02d', $h, $m, $s) }}
                    </h1>
                    <div class="text-white-50 small">
                        {{ \Carbon\Carbon::parse($clockIn)->format('h:i:s A') }} – {{ \Carbon\Carbon::parse($clockOut)->format('h:i:s A') }}
                    </div>
                </div>
            </div>
        @else
            ...
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body">
                    ...
                    @if($staff->shift_start_time && $staff->shift_end_time)
                        ...
                        <h4 class="fw-bold text-muted mb-2">to {{ \Carbon\Carbon::parse($staff->shift_end_time)->format('h:i A') }}</h4>
                    @else
